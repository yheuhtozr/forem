class ChatChannelMembershipsController < ApplicationController
  ...
  def join_channel
    ...
    if status
      render json: { status: "success", message: R18n.t.v.chat.join.request.success }, status: :ok
    else
      render json: { status: 400, message: R18n.t.v.chat.join.request.failure }, status: :bad_request
    end
  end
  ...
  def remove_membership
    ...
    if params[:status] == "pending"
      ...
      message = R18n.t.v.chat.join.pending.remove
    else
      membership = @chat_channel_membership
      message = R18n.t.v.chat.kick.chat(
      ...
      message = R18n.t.v.chat.kick.success user: @chat_channel_membership.user.name
    end
  ...
  def update_membership
    ...
    if @chat_channel_membership.errors.any?
      render json: { success: false, errors: @chat_channel_membership.errors.full_messages,
                     message: R18n.t.v.chat.update.failure }, status: :bad_request
    else
      render json: { success: true, message: R18n.t.v.chat.update.success }, status: :ok
    end
  end
  ...
  def leave_membership
    ...
    send_chat_action_message(R18n.t.v.chat.leave.chat(
                               user: current_user.username, channel: chat_channel_membership.channel_name,
                             ), current_user, chat_channel_membership.chat_channel_id, "left_channel")
    chat_channel_membership.update(status: "left_channel")
    message = R18n.t.v.chat.leave.success channel: channel_name
    if chat_channel_membership.errors.any?
      render json: { success: false, message: R18n.t.v.chat.leave.failure,
  ...
  def update_membership_role
    ...
    if membership.errors.any?
      render json: {
        success: false,
        message: R18n.t.v.chat.update.role.failure,
        errors: chat_channel_membership.errors.full_messages
      }, status: :bad_request
    else
      role = membership.reload.role
      send_chat_action_message(
        R18n.t.v.chat.update.role.chat(user: membership.user.username, role: role),
        current_user, @chat_channel.id,
        "updated"
      )

      render json: { success: true, message: R18n.t.v.chat.update.role.success }, status: :ok
    end
  end
  ...
  def join_channel_invitation
    ...
    existing_membership = ChatChannelMembership.find_by(user_id: current_user.id, chat_channel_id: @chat_channel.id)
    redirect_to Addressable::URI.parse("/connect/#{@chat_channel.slug}").path if existing_membership&.status == "active"
    ...
    if params[:user_action] == "accept"
      ...
      if !membership
        ...
        unless membership&.errors&.any?
          send_chat_action_message(R18n.t.v.chat.join.chat(user: membership.user.username), current_user,
  ...
  def respond_to_invitation(previous_status)
    if permitted_params[:user_action] == "accept"
      ...
      if previous_status == "pending"
        send_chat_action_message(
          R18n.t.v.chat.join.pending.chat(user: current_user.username, channel: @chat_channel_membership.channel_name),
          current_user,
          @chat_channel_membership.chat_channel_id,
          "joined",
        )

        notice = R18n.t.v.chat.join.pending.notice channel: channel_name
      else
        send_chat_action_message(
          R18n.t.v.chat.join.response.chat(user: current_user.username, target: @chat_channel_membership.user.username),
          current_user,
          @chat_channel_membership.chat_channel_id,
          "joined",
        )
        ...
        notice = R18n.t.v.chat.join.response.notice user: @chat_channel_membership.user.username, channel: channel_name
      end
    else
      @chat_channel_membership.update(status: "rejected")
      notice = R18n.t.v.chat.join.response.reject
    end
  ...
  def user_not_authorized
    render json: { success: false, message: R18n.t.v.chat.join.error.unauthorized }, status: :unauthorized
  end

  def record_not_found
    render json: { success: false, message: R18n.t.v.chat.join.error.not_found }, status: :not_found
  end
end
