class ListingTag < LiquidTagBase
  ...
  def get_listing(url)
    hash = get_hash(url)
    raise StandardError, I18n.t("liquid_tags.listing_tag.invalid_url_or_slug_listin") if hash.nil?

    listing = Listing.in_category(hash[:category]).find_by(slug: hash[:slug])
    raise StandardError, I18n.t("liquid_tags.listing_tag.invalid_url_or_slug_listin") unless listing
